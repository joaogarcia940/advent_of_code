...
namespace day4 {
            ...
            for (auto &line: text) {
                ...
                for (auto &letter: line) {
                    if (IsLetterValid(letter)) {
                        nodes.emplace_back(Node{letter, cursor_x, cursor_y, {}});
                    }
                    ++cursor_x;
                }
                ++cursor_y;
    ...
    class Graph {
    public:
        explicit Graph(Nodes &&nodes) : x_nodes_{ExtractNodesByLetter(nodes, 'X')},
                                        m_nodes_{ExtractNodesByLetter(nodes, 'M')},
                                        a_nodes_{ExtractNodesByLetter(nodes, 'A')},
                                        s_nodes_{ExtractNodesByLetter(nodes, 'S')},
                                        part_1_count_{0},
                                        part_2_count_{0} {
            std::vector<ReadingDirection> all_reading_directions = {
                    ReadingDirection::Right, ReadingDirection::Left, ReadingDirection::Up, ReadingDirection::Down,
                    ReadingDirection::UpRight, ReadingDirection::UpLeft, ReadingDirection::DownRight,
        ...
        long GetPart1Count() {
            return part_1_count_;
        }

        long GetPart2Count() {
            return part_2_count_;
        }
    ...
    private:
        ...
        static ReadingDirection GetReadingDirection(const Node &node_1, const Node &node_2) {
        ...
        static bool
        IsNodeOneConnectedToNodeTwo(const Node &node_1, const Node &node_2, ReadingDirection reading_direction) {
        ...
        static void InsertConnectionsBetween(Node &node_1, Nodes &nodes_2, ReadingDirection reading_direction) {
            for (auto &node_2: nodes_2) {
                if (IsNodeOneConnectedToNodeTwo(node_1, node_2, reading_direction)) {
                    node_1.next_nodes.push_back(&node_2);
                }
            ...
            for (auto &origin_node: origin_nodes) // x
            {
                ...
                for (auto next_node_m: origin_node.next_nodes) // m
                {
                    ...
                    for (auto next_node_a: next_node_m->next_nodes) // a
                    {
                        ...
                        {
                            std::cout << "found path: " <<
                                      origin_node.letter << "(" << origin_node.x << "," << origin_node.y << "), "
                                      <<
                                      next_node_m->letter << "(" << next_node_m->x << "," << next_node_m->y << "), "
                                      <<
                                      next_node_a->letter << "(" << next_node_a->x << "," << next_node_a->y << "), "
                                      <<
                                      next_node_s->letter << "(" << next_node_s->x << "," << next_node_s->y << ") | ";
                            ++counter;
                        }
                    }
                }
        ...
        long FindPart2XPathFrom(const Node &a_node) {
            long counter = 0;
            ...
            for (auto next_node: a_node.next_nodes) {
                if (next_node->letter == 'M') {
                    ...
                    auto letter_s_in_the_same_direction = std::find_if(a_node.next_nodes.cbegin(), a_node.next_nodes.cend(),
                                 [&a_node, reading_direction](const auto node) {
                                     return node->letter == 'S' &&
                                            GetReadingDirection(a_node, *node) == reading_direction;
                                 });
